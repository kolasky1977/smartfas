...
original_author="kolasky1977"
original_name="smartfas"
original_urlname="smartfas"
original_description="Awesome smartfas created by kolasky1977" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/smartfas "${name}"
...
then
    ...
    echo "then run 'smartfas' or 'python -m smartfas'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'smartfas' or 'python -m smartfas' or 'poetry run smartfas'"
fi
